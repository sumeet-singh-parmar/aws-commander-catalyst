 ...
 */
async function listAlarms(region, stateValue = null) {
 ...
 */
async function getActiveAlarms(region) {
    return await listAlarms(region, 'ALARM');
}
...
async function getAlarmsSummary(region) {
    const alarms = await listAlarms(region);
