...
### Catalyst Data Store: `user_consent`

Tracks per-user consent for paid AWS services.

| Column | Type | Required | Unique | Description |
|--------|------|----------|--------|-------------|
| `ROWID` | Auto | Yes | Yes | System ID |
| `user_id` | Single Line | Yes | No | Zoho user ID |
| `category_id` | Single Line | Yes | No | Paid category (cost_explorer, bedrock_ai, etc.) |
| `category_name` | Single Line | No | No | Human-readable name |
| `consent` | Boolean | Yes | No | true = consented, false = revoked |
| `consented_at` | Date-Time | Yes | No | When consent was given/revoked |
| `user_name` | Single Line | No | No | User's display name |
| `user_email` | Single Line | No | No | User's email |

**Unique Constraint:** `user_id` + `category_id` combination should be unique.

### Paid Categories

| Category ID | Name | Cost |
|-------------|------|------|
| `cost_explorer` | Cost Explorer | $0.01 per API call |
| `bedrock_ai` | Bedrock AI (Claude) | ~$0.01-0.15 per query |
| `lambda_invoke` | Lambda Invocation | Varies |
| `sns_publish` | SNS Notifications | $0.50/1M + SMS |
